...
file_path = "data.json" # filsti for lagring av data
...
class Library: # klasse for bibliotek
        ...
        for book in self.data["books"]: # iterere gjennom alle bøker
            if book["title"].lower() == title.lower(): # sjekke om tittelen matcher (case-insensitive)
                if book["availability"]: # sjekke om boken er tilgjengelig
                    ...
                    return False # hvis boken er utlånt
    ...
    def search_book(self, title): # metode for å søke etter en bok
        for book in self.data["books"]: # iterere gjennom alle bøker
            if title.lower() in book["title"].lower() or title.lower() in book["author"].lower(): # sjekke om tittelen eller forfatteren matcher (case-insensitive)
                return book # returnere bokdetaljer hvis funnet
        return None # hvis boken ikke ble funnet
...
class LibraryGUI: # klasse for GUI
    def __init__(self, root, library): # initialiseringsmetode
        ...
        tk.Button(root, text="Søk bok", command=self.search_book).pack(pady=2) # knapp for å søke bok
        ...
        tk.Button(root, text="Vis alle bøker", command=self.update_book_list).pack(pady=2) # knapp for å vise alle bøker
    ...
    def search_book(self):
        ...
        if book:
            status = "Tilgjengelig" if book["availability"] else "Utlånt"
            messagebox.showinfo("Søkeresultat", f'{book["title"]} av {book["author"]} {status}')
        else:
            messagebox.showwarning("Ikke funnet", "Boken finnes ikke.") 
        
    def checkout_book(self): # låne bok
        title = self.entry.get() # be om boktittel
        result = self.library.checkout_book(title) # låne bok
        if result is True:
            messagebox.showinfo("Suksess", f'Du har lånt "{title}".') # bekreftelse på lån
        elif result is False:
            messagebox.showwarning("Opptatt", f'"{title}" er allerede utlånt.') # hvis boken er utlånt
        else:
            messagebox.showerror("Feil", "Boken finnes ikke.") # hvis boken ikke ble funnet
        self.update_book_list() # oppdatere bokliste etter lån
 

    def return_book(self): # returnere bok
        title = self.entry.get() # be om boktittel
        result = self.library.return_book(title) # returnere bok
        if result is True:
            messagebox.showinfo("Suksess", f'Du har levert tilbake "{title}".') # bekreftelse på
        elif result is False:
            messagebox.showwarning("Ikke utlånt", f'"{title}" var ikke utlånt.') # hvis boken ikke var utlånt
        else:
            messagebox.showerror("Feil", "Boken finnes ikke.") # hvis boken ikke ble funnet
        self.update_book_list() # oppdatere bokliste etter retur
